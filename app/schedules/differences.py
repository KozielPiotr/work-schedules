...
class Differences:
    """Finds differences between individual schedules"""

    def create_change_messages(self, unaccepted_ind, last_accepted_ind, basic):
        """
    ...
    def compare_inds(self, schedule_id, last_accepted_id, unaccepted_ind, year, month):
        """
        ...
        """
        changes = {}
        workers = self.find_workers(unaccepted_ind)
        cal = Calendar()
                    ...
                    if unaccepted_ind and last_accepted_ind:
                        if unaccepted_ind.begin_hour != last_accepted_ind.begin_hour or \
                           ...
                           unaccepted_ind.event != last_accepted_ind.event:
                            ...
                            msg.append(self.create_change_messages(unaccepted_ind, last_accepted_ind, basic))
            changes[worker] = msg

        return changes

    def get_diff(self, schedule_id):
        """
        ...
        found_diffs = self.compare_inds(schedule_id, last_accepted_id, unaccepted_ind, year, month)
